...
(cl:defmacro test-group (cl:&rest args)
  `(begin ,@args))
...
(define-syntax test-error
  (syntax-rules ()
    ((test-error x)
     (cl:assert (cl:typep (cl:nth-value 1 (cl:ignore-errors x))
                          'cl:error)))))

(test-group "lseqs"
  (test-group "lseqs/constructor"
    (cl:define-symbol-macro one23 (make-lseq 1 2 3))
    (test 1 (car one23))
  ...
  (test-group "lseqs/whole"
    ...
    (cl:defparameter *g* (lseq->generator '(1 2 3)))
    (begin
      (test 1 (_*g*))
      (test 2 (_*g*))
      (test 3 (_*g*))
      (test-assert (eof-object? (_*g*))))
    (cl:setq *g* (lseq->generator (make-lseq 1 2 3)))
    (begin
      (test 1 (_*g*))
      (test 2 (_*g*))
      (test 3 (_*g*))
    ...
    (cl:define-symbol-macro one23abc (lseq-append (make-lseq 1 2 3) (make-lseq 'a 'b 'c)))
    (test-assert (procedure? (cdr one23abc)))
    (test-assert (lseq-realize one23abc))

    (cl:define-symbol-macro one2345 (make-lseq 1 2 3 4 5))
    (cl:define-symbol-macro oddeven (make-lseq 'odd 'even 'odd 'even 'odd 'even 'odd 'even))
  ...
  (test-group "lseqs/mapping"
    ...
    (cl:defparameter *output* ())
    (cl:defparameter out! (lambda (x) (set! *output* (cons x *output*))))
    (lseq-for-each out! '())
    (test *output* '())
    ...
    (cl:defparameter *odds* (lseq-filter #'odd? '(1 2 3 4 5)))
    ...
    (cl:setq *odds* (lseq-remove #'even? '(1 2 3 4 5)))
    (test-assert (procedure? (cdr *odds*)))
    (test '(1 3 5) (lseq-realize *odds*))
